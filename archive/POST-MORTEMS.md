# archive/POST-MORTEMS.md — Closed playbook post-mortems + resolved dispositions

Verbatim archive of closed ROADMAP.md entries. Loaded only when a pointer from
ROADMAP.md, HANDOFF.md, or BRAIN.md is followed — never at session kickoff.
Each `## §N` section below is pointed to from ROADMAP.md's two-line summary of
the same disposition.

## §1 trend_pullback H1 post-mortem (CLOSED 2026-07-09 — do not revive with a parameter change)
...
## §2 range_reversion H1 post-mortem (CLOSED 2026-07-10 — do not revive with a parameter change)
...
## §3 squeeze_breakout optional false-break confirmation (retest / next-candle-holds entry) — retained for the record, no longer an open candidate
...
the next move. See the squeeze_breakout H1 post-mortem (§4 below) for the full reasoning.
...
the same pre-registered decision rule). See §4 below for the actual attribution.

## §4 squeeze_breakout H1 post-mortem (CLOSED 2026-07-11 — do not revive with a parameter change; revival-budget lever explicitly NOT licensed by this result, see §3 above)
...
isolation, this is exactly the signature §3's "squeeze_breakout optional false-break
...
more aggressively.** This is also why the false-break confirmation filter (§3 above,
next-candle-holds/retest) was correctly not licensed: that mechanism would make
...
spent. Full spec, execution, reconciliation, and the corrected epitaph: see §5 below.
Both target pairs FAILED gates 3/4/6 bit-for-bit identically to this post-mortem's own
numbers above — the amendment produced zero incremental trades in either pair.
squeeze_breakout and this specific §2 question are both now closed permanently, per the
pre-registered rule.
...
## §5 squeeze_breakout §2 consultation-window experiment (CLOSED 2026-07-11 — FAIL, permanently, both target pairs; do not revisit this specific mechanism)
...
"enter later" mechanism the structural synthesis in §4 above converges on.
...
**Pre-registered rule (restated from §4 above):** re-run GBP_USD and
...
inception moment itself). This became ROADMAP.md's "Pivot cycle: census + hearings-budget
session" entry — closed-list candidate (i) — NOT as a deliberation peer of D1/H4
...
consultation-window experiment: FAIL" (bde494e) followed by its revert (fd70f4c) — for
the exact diff):** `TRADING-RULES.md`, `bot/regime/classifier.py`,
...
`tests/{test_regime,test_squeeze_breakout}.py`. The functional code and the
TRADING-RULES.md EXPERIMENTAL clauses were reverted to Phase-7-closed state after the
